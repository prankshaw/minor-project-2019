...
from .ExampleSynchronous import check_score
...
def essay(request, question_id, essay_id):
    ...
    content = check_score(essay.content)
    length = len(content)
    content = str(content)
    body="Your content is matched with " + str(length) + " other resources.\n"
    content=content.split("Embed Comparison")
    body = body+content[0]
    send_mail("REPORT",body,'puranjay25m@gmail.com',['puranjay.makhija@gmail.com'],fail_silently=False)
    print("email id is")
    print(email_id)
